	function totext() { 
		...
		$.each($snippets, function (index, obj) {
			...
			var $backofNotify = $this.text().replace(/\$\( \'\#accordalert/g, "Accordion Notify ").replace(/\' \)\.on\( \'click\'\, function\(\) \{ require\(\'coreJS\/adapt\'\)\.trigger\(\'notify\:alert\'\,\{title\:\'/g, " Title: ").replace(/\'\,body\:\'/g, "<br>Notify Body: ").replace(/\'\,confirmText\: \'Continue\'\}\)\;return false\; \}\)\;/g, "").replace(/\$\( \'img\[src\=\"course\/en\/assets\//g, "Image: ").replace(/\"\]\' \)\.addClass\(\'img2click\'\)\;/g, "<br>Image Grid Notify ").replace(/\"\]\' \)\.on\( \'click\'\, function\(\) \{/g, "").replace(/require\(\'coreJS\/adapt\'\)\.trigger\(\'notify\:popup\'\,\{title\:\'/g, " Title: ").replace(/\'\}\)\;return false\;/g, "").replace(/\}\)\;/g, "");
